        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
               ...
               (error "MUPL ifgreater applied to non-nmber")))]
        [(fun? e) 
         (closure env e)]
        [(call? e) 
         (let ([clos (eval-under-env (call-funexp e) env)]
               [actu (eval-under-env (call-actual e) env)])
           (if (closure? clos)
               (if (fun-nameopt (closure-fun clos))
                   (eval-under-env 
                    (fun-body (closure-fun clos))
                    (cons (cons (fun-nameopt (closure-fun clos)) clos) 
                          (cons (cons (fun-formal (closure-fun clos)) actu) 
                                (closure-env clos))))
                   (eval-under-env 
                    (fun-body (closure-fun clos))
                    (cons (cons (fun-formal (closure-fun clos)) 
                                actu)
                          (closure-env clos))))
...
(define (mlet* rlstpr e2)
  (cond [(null? rlstpr) e2]
        [(null? (cdr rlstpr)) (mlet (car (car rlstpr)) (cdr (car rlstpr)) e2)] 
        [#t (mlet (car (car rlstpr)) (cdr (car rlstpr)) (mlet* (cdr rlstpr) e2))]))
...
(define mupl-map
  (fun "higher-map" "map-fun" 
       (fun "actual-map" "mlst" 
            (ifgreater (isaunit (var "mlst")) 
                       (int 0) 
                       (aunit) 
                       (apair (call (var "map-fun") (fst (var "mlst"))) 
                              (call (var "actual-map") (snd (var "mlst"))))))))

(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "n" 
             (fun #f "mlst" 
                  (call (call (var "map") (fun #f "x" (add (var "n") (var "x")))) (var "mlst"))))))
